...
PreyThreat::PreyThreat(mt19937_64 &mt, const Chromosome &_chr, const PVector &_parentPos, const ll &_id) : Prey::Prey(mt, _chr, _parentPos, _id)
{
    F_threat = true;
    TakeEnergyRate = _chr.TakeEnergyRate;
}
...
void PreyThreat::RobEnergy(mt19937_64 &mt, Flock &f)
{
    ...
    for (int i = 0; i < f.flock.size(); i++)
    {
        if (f.flock[i]->F_live && !f.flock[i]->F_predator && f.flock[i]->Energy > 0)
        {
            ...
            if (firstID != secondID)
            {
                ...
                {
                    CandOfTarget.push_back(f.flock[i]->ID);
                }
            }
        }
    }
    ...
    if (CandOfTarget.size() > 0)
    {
        ...
        if (f.flock[index]->Energy > TakeEnergyRate * INIT_MEAN_ENERGY)
        {
        ...
        {
            f.flock[index]->Energy = 0;
            Energy += f.flock[index]->Energy;
        }
